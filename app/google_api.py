...
def build_service(
    ...
    Builds a fresh client on every call, deliberately -- the underlying
    ``httplib2``/``googleapiclient`` transport is not thread-safe, and this
    app calls Gmail/Sheets/Drive from several different threads (request
    handlers, the real-time poller, the digest scheduler). A cached, shared
    client caused connection-pool corruption (``'NoneType' object has no
    attribute 'close'``) the first time the automatic poller and a manual
    request overlapped. See git history for the reverted caching attempt.
