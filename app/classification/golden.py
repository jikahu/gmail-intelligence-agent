...
to catch a classifier regression before it ships.

A golden dataset is a small, hand-labeled, checked-in fixture set — so it can
run in every ``pytest`` pass and act as a permanent regression test. The
actual dataset content lives in ``tests/golden_dataset/`` (a *test* fixture);
...
Metrics: overall accuracy, a per-category breakdown, and — the one that
matters most — the protected-email false-Review rate, which must be zero
(CLAUDE.md §8/§21: an email wrongly hidden in Review is the one mistake this
app must never make).
"""
